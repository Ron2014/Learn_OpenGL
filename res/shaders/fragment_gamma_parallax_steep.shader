...
vec2 ParallaxMapping(vec2 texCoord, vec3 viewDir){
    ...
    float currentDepthMapValue = texture(material.displace, currentTexCoords).r;

    while(currentLayerDepth < currentDepthMapValue) {
        // shift texture coordinates along direction of P
        currentTexCoords -= deltaTexCoords;
        // get depthmap value at current texture coordinates
        currentDepthMapValue = texture(material.displace, currentTexCoords).r;  
    ...
    float beforeDepth = texture(material.displace, prevTexCoords).r - currentLayerDepth + layerDepth;
    ...
    // return texCoord - currentTexCoords;
}
...
void main()
{
  ...
  lighting += CalcDirectLight(directLight, normal, viewDir);
  // lighting += CalcPointLight(pointLight, normal, viewDir, fs_in.FragPos);
  lighting += CalcPointLight(pointLight, normal, viewDir, fs_in.TangentFragPos);
  texColor *= lighting;
