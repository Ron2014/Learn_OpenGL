...
extern vec3 pointLightPos(-3.0f, 0.0f, -2.0f);
...
void initShaders() {
  ...
  shader[IDX_PLANE] = new Shader("vertex_reverse_normal.shader", "fragment_gamma_all_shadow.shader");
  ...
  shader[IDX_DBUG] = new Shader("vertex_debug_cubemap_depth.shader", "fragment_debug_cubemap_depth.shader");     // 调试用
