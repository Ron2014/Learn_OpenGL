...
void update(float updateTime) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO[IDX_CUBEMAP_DEPTH]);
    glBufferData(GL_ARRAY_BUFFER, FACE_SIZE * sizeof(glm::mat4), &pointMatrix[0], GL_DYNAMIC_DRAW);

    // to plane
    cubemapAttrib(VAO[IDX_PLANE]);

    // to cube
    cubemapAttrib(VAO[IDX_CUBE]);

    // to model
    for (int i=0; i<model_count; i++) {
      for (Mesh mesh : loading_models[i]->GetMesh()) {
        cubemapAttrib(mesh.VAO);
      }
    }
