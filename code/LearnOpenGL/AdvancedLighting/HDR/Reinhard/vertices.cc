...
void initCubeData() {
  ...
  tex_fb[FB_DEPTH_COLOR] = initFrameBufferColor(FRAME_BUFFER[FB_DEPTH_COLOR], "hdrBuffer");
...
void drawQuad() {
  Texture2D::use({tex_fb[FB_DEPTH_COLOR]}, shader[IDX_COLOR_BUF_R]);
  shader[IDX_COLOR_BUF_R]->setInt("hdr", hdr);
  shader[IDX_COLOR_BUF_R]->setFloat("exposure", exposure);
