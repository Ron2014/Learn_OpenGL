...
int main(int argc, char *argv[]) {
  ...
  GLFWwindow* window = glfwCreateWindow(WIN_WIDTH, WIN_HEIGHT, "Mateirals", NULL, NULL);
  if (window == NULL)
  {
  ...
  shader[IDX_OBJ] = new Shader("vertex_specular.shader", "fragment_material.shader");
  ...
  vec3 ambientColor(0.1f, 0.05f, 0.031f);
  vec3 diffuseColor(1.0f, 0.5f, 0.31f);
  vec3 specularColor(0.5f, 0.5f, 0.5f);
  float shininess = 256.0f;
  shader[IDX_OBJ]->setVec3("material.ambient", glm::value_ptr(ambientColor));
  shader[IDX_OBJ]->setVec3("material.diffuse", glm::value_ptr(diffuseColor));
  shader[IDX_OBJ]->setVec3("material.specular", glm::value_ptr(specularColor));
