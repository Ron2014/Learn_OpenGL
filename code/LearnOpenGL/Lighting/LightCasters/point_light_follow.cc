...
int main(int argc, char *argv[]) {
  ...
  shader[IDX_OBJ] = new Shader("vertex_specular.shader", "fragment_point_light.shader");
  ...
  while(!glfwWindowShouldClose(window))
  {
    ...
    // glm::vec3 lightPos = camera->Position + camera->Up;                              // 头顶灯泡
    glm::vec3 lightPos = camera->Position + camera->Right + camera->Front * 3.0f;       // 右手持煤油灯
